...
INPUT_FILES = {
		...
		"failures": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0", "tmQMg-L", "no_coverage.txt"),
        ...
        "details": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF-1Ksel", "intermediate", "v1.0", "details.csv")
	}

OUTPUT_FILES = {
		"centres": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "tmQMg_centres.txt"),
		"coverage_a": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "coverage_a.txt"),
		"coverage_l": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "coverage_l.txt"),
        "atoms": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "atoms.txt"),
        "latmods": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "latmods.txt"),
        "electronic": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "electronic.txt"),
		"figures": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "summary", "v1.0", "failure_inspection", "figures")
	}
...
def plot_atoms_and_sizes(atoms_and_sizes):
    pubchem = pd.read_csv(INPUT_FILES["periodic_table"], sep = ",")
    ...
    x = np.arange(len(atoms["tmQMg"]))
    
    width = 0.7

    for ax, key, col in zip(axs, keys, ["green", "red", "blue"]):
        # Centres distribution (normalised) in tmQMg
        count = {
            el: c 
            for el, c in sorted(atoms[key].items(), key = lambda ec: pubchem.loc[pubchem["Symbol"] == ec[0], "AtomicNumber"].iloc[0])
        }
        Z = sum(count.values())
        ## Plot tmQMg
        ax.bar(x, np.array(list(count.values()))/Z, width = width, color = col, alpha = 0.3)
        ax.set_yscale("log")
        ...
        ax.set_ylabel(f"Normalised counts ({key})")

    fig.savefig(os.path.join(OUTPUT_FILES["figures"], "elements.pdf"), format = "pdf", bbox_inches = "tight")
    fig.savefig(os.path.join(OUTPUT_FILES["figures"], "elements.png"), format = "png", bbox_inches = "tight", dpi = 200)
    ...
    for ax, key, col in zip(axs, keys, ["green", "red", "blue"]):
        ...
        ## Plot tmQMg
        ax.hist(sizes[key], color = col, alpha = 0.3)
    ...
    fig.savefig(os.path.join(OUTPUT_FILES["figures"], "sizes.pdf"), format = "pdf", bbox_inches = "tight")
    fig.savefig(os.path.join(OUTPUT_FILES["figures"], "sizes.png"), format = "png", bbox_inches = "tight", dpi = 200)
...
if __name__ == "__main__":
    ...
    if INSPECT_ATOMS:
        ...
        plot_atoms_and_sizes(atoms_and_sizes)
