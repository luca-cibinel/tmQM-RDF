...
INPUT_FILES = {
        "localised_tmcs": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQMg", "localised_tmcs.csv"),
        ...
        "viable": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "step_2_i_viable_tmcs.txt")
	}
...
def validate_bo_info(tmc_id, bo_info):
    ...
    g = nx.read_gml(os.path.join(INPUT_FILES["tmQMg_graphs"], tmc_id + ".gml"))
...
def build_graphs(atlas, resume = True):
    ...
    for tmc in tqdm(viables):
        ...
        Write graph to .gml
        """
        nx.write_gml(g, os.path.join(OUTPUT_FILES["tmQM_graphs"], tmc + ".gml"))
