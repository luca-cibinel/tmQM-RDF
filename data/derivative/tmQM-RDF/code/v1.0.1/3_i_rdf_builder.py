...
INPUT_FILES = {
		"viable": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "viable_tmcs.txt"),
		"ligands_atoms": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQMg-L", "ligands_atoms_idx.csv"),
		...
		"tmQM_graphs": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQM", "uNatQ_graphs"),
		"tmQMg_graphs": os.path.join(ROOT_DIR, "data", "raw", "tmQMseries", "data", "tmQMg", "v74.637k", "uNatQ_graphs"),
		"tmQMg_L_graphs": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQMg-L", "uNatQ_graphs")
	}

OUTPUT_FILES = {
		"tmQM_RDF_ABox": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "data", "v1.0.1", "assertions"),
        "tmQM_RDF_TBox": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "data", "v1.0.1", "terminology")
	}
...
def read_global_files():
    ...
    
    {
        (property_key, reference_db): [
                [nm("optimisation"), nm(optimisation)],
                [nm("singlepoint"), nm(singlepoint)]
            ]
    }
    """
    tmQM_series_property_table = defaultdict(list)
    
    csv_property_table = pd.read_csv(
            INPUT_FILES["tmQM_series_properties"], 
            sep = ";",
            converters = {
                "optimisation": str,
                "singlepoint": str
    ...
    for i in range(len(csv_property_table)):
        ...
        tmQM_series_property_table[(row.iloc[0], row.iloc[1])] = []

        if row.iloc[2]:
            tmQM_series_property_table[(row.iloc[0], row.iloc[1])] += [
                [pfx.nm("optimisation"), pfx.nm(row.iloc[2].replace("-", "_").replace("/", "__"))]
            ]
        if row.iloc[3]:
            tmQM_series_property_table[(row.iloc[0], row.iloc[1])] += [
                [pfx.nm("singlepoint"), pfx.nm(row.iloc[3].replace("-", "_").replace("/", "__"))]
            ]
...
def helper_write_property(property_type, reference_db, value, method = [], indent = 1):
    ...
    Helper function: writes a property (assuming that the subject is already implicit):
        ms:hasProperty [
            ...
            ms:reference reference_db ;
            rdf:value "value"^^xmls:data_type ;
        ] ;
    ...
    return helper_write_blank_node_sentence(pfx.ms("hasProperty"), [
            [pfx.rdf("type"), property_type],
            [pfx.ms("reference"), reference_db],
...
#%%% ABox
def build_atomic_level(tmc_id, tmQM_g, tmQMg_g, tmQM_series_property_table):
    """
    ...
    for at_id, uri_atom in atom_atlas.items():
        ...
        for p, v in tmQMg_atomic_properties[at_id].items():
            ...
            triples += helper_write_property(
                ...
                pfx.dsG("tmQMg"),
                v,
				tmQM_series_property_table[p_name, "tmQMg"]
            )
        ...
        xyz_triples += [
            ...
            pfx.ms("reference") + " " + pfx.dsG("tmQMg") + " ;"
        ] + [
			" ".join(entry) + " ;" for entry in tmQM_series_property_table["node_position", "tmQMg"]
		]
        
        xyz_triples += helper_write_blank_node_sentence(pfx.rdf("value"), 
            [[pfx.rdf("type"), pfx.ms("CartesianCoordinates3D")]] + [
                [
                    pfx.tmAp("xyz"[i]), 
        ...
        triples += helper_write_blank_node_sentence(
            pfx.ms("hasProperty"), xyz_triples, raw = True
        )
        ...
        for p, v in tmQM_atomic_properties[at_id].items():
            ...
            triples += helper_write_property(
                ...
                pfx.dsC("tmQM"),
                v,
				tmQM_series_property_table[p_name, "tmQM"]
            )
        ...
        xyz_triples += [
            pfx.rdf("type") + " " + pfx.tmAp("node_position") + " ;",
            pfx.ms("reference") + " " + pfx.dsC("tmQM") + " ;"
        ] + [
			" ".join(entry) + " ;" for entry in tmQM_series_property_table["node_position", "tmQM"]
		]
    ...
    for e in bonds_atlas:
        ...
        triples += helper_write_blank_node_sentence(
            pfx.ms("hasProperty"), [
				...
				" ".join(entry) + " ;" for entry in tmQM_series_property_table["nbo_type", "tmQMg"]
			], raw = True)
...
def build_complex_level(uri_tmc, tmQM_g, tmQMg_g, local_ligand_atlas, uri_metal_centre, tmQM_series_property_table):
    ...
    """
    meta_triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), [
    ...
    count_triples += [pfx.ms("reference") + " " + pfx.dsG("tmQMg") + " ;"]
    
    bag_triples = []
    bag_triples += [pfx.rdf("type") + " " + pfx.ms("CountDescription") + " ;"]
    for i, el_count in enumerate(tmQMg_g.graph["meta_data"]["element_counts"].items()):
        bag_triples += helper_write_blank_node_sentence(pfx.rdf(f"_{i + 1}"), [
                pfx.rdf("type") + " " + pfx.ms("CountDescriptionItem") + " ;",
                pfx.ms("countOf") + " " + pfx.tmAr(el_count[0]) + " ;",
    ...
    meta_triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), count_triples, raw = True, indent = 0)
...
def assemble_tmc(viables, ligands_atlas, tmQM_series_property_table):
    """
    ...
    """
    if not os.path.exists(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "TMCs")):
        os.makedirs(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "TMCs"))
    ...
    for tmc in tqdm(viables):
        ...
        Write to file
        """
        with open(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "TMCs", tmc + ".ttl"), "w") as f:
            f.write("\n".join(triples))
...
def assemble_mc_and_ligands(ligands_misc, ligands_fingerprints, ligands_descriptors, reference_ligand_atlas, reference_centres_atlas, subgraph_atlas, tmQM_series_property_table, periodic_table):
    """
    Builds an ontology with all the reference information of the metal centres and ligands used in tmQM-RDF.
    Each ligand and metal centre is written to a separate file.
    ...
    if not os.path.exists(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "ligands")):
        os.makedirs(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "ligands"))
        
    if not os.path.exists(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "centres")):
        os.makedirs(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "centres"))
    
    property_atlas = defaultdict(list)
    
    start_time = time.time()
    print("Building centres and ligands assertions...")
    ...
    for uri_centre, centre_el in tqdm(sorted(reference_centres_atlas.items(), key = atnum)):
        ...
        """
        Add prefixes (not all are needed, its likely going to be a small file)
        """
        required_prefixes = set(re.findall(r'(?<=\W)\w+(?=:)', " " + "\n".join(triples)))
        
        prefixes = []
        for p in [getattr(pfx, prefix) for prefix in set(required_prefixes)]:
            prefixes += ["@prefix " + p("") + " <" + p() + "> ."]
        
        """
        Write to file
        """
        with open(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "centres", f"MetalCentre_{centre_el}.ttl"), "w") as f:
    ...
    for uri_lig, lig_id in tqdm(sorted(reference_ligand_atlas.items(), key = ligsize)):
        ...
        value_triples += [pfx.ms("reference") + " " + pfx.dsL("tmQMg-L") + " ;"]
        value_triples += helper_write_blank_node_sentence(pfx.rdf("value"), haptic_group_triples, raw = True, indent = 0)
        
        triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), value_triples, raw = True)
        
        property_atlas[pfx.lgLrp] += ["smiles_metal_bond_node_idx_groups"]
        
        """
        Add bond orders (denticity/hapticity)
        """
        kappa = len([site for site in binding_atoms if len(site) == 1])
        etas = [len(site) for site in binding_atoms if len(site) > 1]

        bond_order_triples = [pfx.rdf("type") + " " + pfx.lgLr("DenticityHapticityOrderList") + " ;"]
        bond_order_triples += helper_write_blank_node_sentence(
                pfx.rdf("_1"),
                [[pfx.rdf("type"), pfx.lgLr("Kappa")]] +
                [[pfx.rdf("value"), helper_typed_literal(kappa)]],
                indent = 0
            )
        for i, eta in enumerate(etas):
            bond_order_triples += helper_write_blank_node_sentence(
                    pfx.rdf(f"_{i + 2}"),
                    [[pfx.rdf("type"), pfx.lgLr("Eta")]] +
                    [[pfx.rdf("value"), helper_typed_literal(eta)]],
                    indent = 0
                )

        value_triples = []
        value_triples += [pfx.rdf("type") + " " + pfx.lgLrp("denticity_hapticity_orders") + " ;"]
        value_triples += [pfx.ms("reference") + " " + pfx.dsL("tmQMg-L") + " ;"]
        value_triples += helper_write_blank_node_sentence(pfx.rdf("value"), bond_order_triples, raw = True, indent = 0)


        triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), value_triples, raw = True)

        property_atlas[pfx.lgLrp] += ["denticity_hapticity_orders"]
        """
        ...
        Add ligand fingerprints:
            ...
            - haptic atom count
        """
        atoms_to_count = [sym for sym in ligands_fingerprints.columns[7:(7+14)] if sym in list(periodic_table.loc[:, "Symbol"])]
        ...
        for count_type in ["", "dentic_", "haptic_"]:
            ...
            count_triples += [pfx.ms("reference") + " " + pfx.dsL("tmQMg-L") + " ;"]
            ...
            for i, el_count in enumerate(atoms_count.items()):
                bag_triples += helper_write_blank_node_sentence(pfx.rdf(f"_{i + 1}"), [
                        pfx.rdf("type") + " " + pfx.ms("CountDescriptionItem") + " ;",
                        pfx.ms("countOf") + " " + pfx.tmAr(el_count[0].replace(count_type, "")) + " ;",
                        pfx.rdf("value") + " " + helper_typed_literal(el_count[1]) + " ;"
                    ], raw = True, indent = 0)
            ...
            """
            triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), count_triples, raw = True)
        ...
        Add ligand descriptors:
            - stable occurence
        """
        stable_occurrence_subgraph = ligands_misc.loc[ligands_misc["name"] == lig_id, "stable_occurrence"]
        ...
        stable_occurrence_triples += [pfx.ms("reference") + " " + pfx.dsL("tmQMg-L")  + " ;"]
        ...
        triples += helper_write_blank_node_sentence(pfx.ms("hasProperty"), [
                        [pfx.rdf("type"), pfx.lgLrp("logp")],
                        [pfx.ms("reference"), pfx.dsL("tmQMg-L")],
        ...
        for i, str_count in enumerate(struct_count.items()):
            bag_triples += helper_write_blank_node_sentence(pfx.rdf(f"_{i + 1}"), [
                    pfx.rdf("type") + " " + pfx.ms("CountDescriptionItem") + " ;",
                    pfx.ms("countOf") + " " + pfx.lgLrm(to_class_name(str_count[0])) + " ;",
        ...
        Write to file
        """
        with open(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "ligands", f"{lig_id}.ttl"), "w") as f:
            f.write("\n".join(triples))
...
def assemble_elements(reference_element_set, periodic_table):
    ...
    Each element is written to a separate file.
    ...
    Returns:
        - the property_atlas {prefix: set([properties]), ...} of the properties encountered during the construction
    """
    if not os.path.exists(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "elements")):
        os.makedirs(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "elements"))
    ...
    for el in sorted(reference_element_set, key = atnum):
        ...
        Write to file
        """
        with open(os.path.join(OUTPUT_FILES["tmQM_RDF_ABox"], "elements", f"{el}.ttl"), "w") as f:
            f.write("\n".join(prefixes + triples))
...
def build_classes_ontology():
    ...
    classes = {
            ...
            pfx.lgLr("DenticityHapticityOrderList"): [
                    [pfx.rdf("Bag")], 
                    "List of denticity/hapticity orders of ligand",
                    "This class represents a list containing denticity and/or hapticity orders for the ligand, as appropriate."
                ],
            pfx.lgLr("Kappa"): [
                    [], 
                    "Denticity order",
                    "This class represents the denticity order (kappa) of a ligand."
                ],
            pfx.lgLr("Eta"): [
                    [], 
                    "Hapticity order",
                    "This class represents the hapticity order (eta) of a ligand."
                ],
            ...
            pfx.ms("CartesianCoordinates3D"): [
                    [pfx.ms("NonSimplePropertyValue"), pfx.rdf("Seq")], 
            ...
            pfx.ms("CountDescription"): [
                    [pfx.ms("NonSimplePropertyValue"), pfx.rdf("Bag")], 
                    "Count description",
    ...
    """
    triples_by_prefix = defaultdict(list)
    
    for class_name, class_body in classes.items():
        prefix = class_name.split(":")[0]
        
        triples_by_prefix[prefix] += [f"\n{class_name}"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdf("type") + " " + pfx.rdfs("Class") + " ;"]
        
        if len(class_body[0]) > 0:
            triples_by_prefix[prefix] += ["\t" + pfx.rdfs("subClassOf") + " " + ", ".join(class_body[0]) + " ;"]
            
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("label") + " " + helper_typed_literal(class_body[1]) + " ;"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("comment") + " " + helper_typed_literal(class_body[2]) + " ;"]
        
        triples_by_prefix[prefix] += ["."]
            
    return triples_by_prefix
...
def build_predicates_ontology():
    ...
    for pred_name, pred_body in predicates.items():
        prefix = pred_name.split(":")[0]
        
        triples_by_prefix[prefix] += [f"\n{pred_name}"]
        
        if len(pred_body[0]) > 0:
            triples_by_prefix[prefix] += ["\t" + pfx.rdf("type") + " " + ", ".join(pred_body[0]) + " ;"]
            
        if len(pred_body[1]) > 0:
            triples_by_prefix[prefix] += ["\t" + pfx.rdfs("subPropertyOf") + " " + ", ".join(pred_body[1]) + " ;"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("domain") + " " + pred_body[2] + " ;"]
    
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("range") + " " + pred_body[3] + " ;"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("label") + " " + helper_typed_literal(pred_body[4]) + " ;"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("comment") + " " + helper_typed_literal(pred_body[5]) + " ;"]
...
def build_misc_reference_ontology():
    ...
    for obj_name, obj_body in objects.items():
        ...
        triples_by_prefix[prefix] += ["\t" + pfx.rdf("type") + " " + ", ".join(obj_body[0]) + " ;"]
            
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("label") + " " + helper_typed_literal(obj_body[1]) + " ;"]
        
        triples_by_prefix[prefix] += ["\t" + pfx.rdfs("comment") + " " + helper_typed_literal(obj_body[2]) + " ;"]
        
        triples_by_prefix[prefix] += ["."]
            
    return triples_by_prefix

def build_dataset_reference_ontology():
    """
    Builds an ontology with all the reference information of various miscellanea (NBO types, countable structural features of ligands,
    ligand occurrence types)
...
def build_property_ontology(tmc_property_atlas, lig_property_atlas, atom_property_atlas):
    ...
    for fprefix, p_set in property_atlas.items():
        prefix = fprefix("")[:-1]
        ...
        for p in p_set:
            ...
            triples_by_prefix[prefix] += [""]
            triples_by_prefix[prefix] += [fprefix(p)]
            
            triples_by_prefix[prefix] += ["\t" + pfx.rdfs("subClassOf") + " " + pfx.ms("ObservedProperty") + " ;"]
            
            triples_by_prefix[prefix] += ["\t" + pfx.rdf("type") + " " + pfx.ms("PropertyType") + " ;"]
                
            triples_by_prefix[prefix] += ["\t" + pfx.rdfs("label") + " " + helper_typed_literal(lab) + " ;"]
            
            triples_by_prefix[prefix] += ["\t" + pfx.rdfs("comment") + " " + helper_typed_literal(desc) + " ;"]
            
            triples_by_prefix[prefix] += ["."]
            
    return triples_by_prefix

def assemble_tbox(tmc_property_atlas, lig_property_atlas, atom_property_atlas):
    """
    ...
    classes_triples = build_classes_ontology()
    predicates_triples = build_predicates_ontology()
    misc_triples = build_misc_reference_ontology()
    dataset_triples = build_dataset_reference_ontology()
    property_triples = build_property_ontology(tmc_property_atlas, lig_property_atlas, atom_property_atlas)
    
    for triples_dict in [classes_triples, predicates_triples, misc_triples, dataset_triples, property_triples]:
        for prefix, triples in triples_dict.items():
            triples_by_prefix[prefix] += triples
    
    for prefix, raw_triples in triples_by_prefix.items():
        triples = []
        ...
        with open(os.path.join(path, f"{prefix}.ttl"), "w") as f:
            f.write("\n".join(triples))
...
if __name__ == "__main__":
    ...
    # ABox
    reference_ligand_atlas, reference_centres_atlas, subgraph_atlas, reference_element_set, tmc_property_atlas = assemble_tmc(viables, ligands_atlas, tmQM_series_property_table)
    lig_property_atlas = assemble_mc_and_ligands(ligands_misc, ligands_fingerprints, ligands_descriptors, reference_ligand_atlas, reference_centres_atlas, subgraph_atlas, tmQM_series_property_table, periodic_table)
    atom_property_atlas = assemble_elements(reference_element_set, periodic_table)
    
    # TBox
    assemble_tbox(tmc_property_atlas, lig_property_atlas, atom_property_atlas)
