...
INPUT_FILES = {
        ...
        "ligands_misc_info": os.path.join(ROOT_DIR, "data", "raw", "tmQMseries", "data", "tmQMg-L", "v74k", "ligands_misc_info.csv"),
        "ligands_atoms": os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQMg-L", "ligands_atoms_idx.csv")
    }

OUTPUT_FILES = {
        "checkpoint":  os.path.join(ROOT_DIR, "data", "derivative", "tmQM-RDF", "intermediate", "v1.0.1", "tmQMg-L", "step_1_ii_checkpoint.txt"),
...
def write_to_csv(uNatQ_graphs):
    ...
    for tmc, content in tqdm(uNatQ_graphs.items()):
        ...
        """
        with open(os.path.join(OUTPUT_FILES["tmQMg_L_graphs"], tmc + ".csv"), "w") as f:
             f.write("\n".join(content))
