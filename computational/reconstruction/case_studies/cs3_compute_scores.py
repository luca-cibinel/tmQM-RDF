...
OUTPUT_FILES = {
        "results": os.path.join(ROOT_DIR, "computational", "reconstruction", "case_studies", "results", "%s", "scores"),
        ...
        for wp in working_paths:
            ...
            with open(os.path.join(INPUT_FILES["bn"] % dataset_tag, wp, SETTINGS["fitted_graph_name"]), "rb") as f:
                ...
                for case_tmc in case_studies:
                    ...
                    recons = [r for r in os.listdir(os.path.join(INPUT_FILES["reconstructions"] % tm_mode, case_tmc)) if r.startswith("recon")]
                    ...
                    for r, recon in tqdm(enumerate(recons)):
                        ...
                        files = os.listdir(os.path.join(INPUT_FILES["reconstructions"] % tm_mode, case_tmc, recon))
                        ...
                        aggregated_matches = aggregate_raw_matches(
                            dataset_tag,
                            os.path.join(INPUT_FILES["reconstructions"] % tm_mode, case_tmc, recon, "matches.csv"),
                            wp
                        )
                    ...
                    results_folder = os.path.join(OUTPUT_FILES["results"] % dataset_tag, wp, case_tmc)
